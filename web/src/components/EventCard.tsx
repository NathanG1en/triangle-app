...
export const EventCard: React.FC<EventCardProps> = ({
  ...
  return (
    <div
      ...
      className="group py-4 border-b border-[#E5E0D8] hover:bg-[#F5F1EC]/60 px-2 rounded-xl transition-all cursor-pointer flex flex-col justify-between gap-3"
    >
      {/* Top Main Row */}
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div
            className={`text-[10px] font-extrabold uppercase tracking-wider mb-1 ${
              isSpot ? 'text-[#075E59]' : 'text-[#D95F4B]'
            }`}
          >
            {categoryCityHeader}
          </div>
          ...
          <h3 className="font-['Bricolage_Grotesque'] text-lg font-bold text-[#1A1A1A] leading-snug group-hover:text-[#D95F4B] transition-colors mb-1.5 line-clamp-2">
            {event.title}
          </h3>

          {/* Metadata Sub-line */}
          <p className="text-xs text-[#77736F] font-semibold truncate mb-2">
            {timePriceVenueLine}
          </p>

          {/* Recurrence Summary */}
          {event.recurrence_rule && (
            <span className="inline-flex items-center gap-1 text-[11px] font-bold text-[#D95F4B] mb-2">
              <Repeat size={11} />
        ...
        {event.image_url ? (
          <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-xl overflow-hidden bg-[#E5E0D8] shrink-0 border border-[#E5E0D8]">
            <img
              src={event.image_url}
              alt={event.title}
      ...
      <div className="flex items-center justify-between gap-2 pt-1 border-t border-[#E5E0D8]/40">
          ...
          <div className="flex items-center shrink-0">
            {goingUsers.slice(0, 3).map((a, idx) => (
              <img
                ...
                alt={a.user_name}
                className="w-5 h-5 rounded-full border-1.5 border-[#FFFEFD] object-cover -ml-1.5 first:ml-0"
              />
            ))}
          </div>
        ...
        <div className="flex items-center gap-1.5 shrink-0" onClick={(e) => e.stopPropagation()}>
          <button
            ...
            className="p-1.5 rounded-lg text-[#77736F] hover:text-[#1A1A1A] hover:bg-[#E5E0D8] transition-colors"
            title="Share"
          >
            <Share2 size={13} />
          </button>
          ...
          <button
            ...
            className={`px-2.5 py-1 rounded-lg text-xs font-bold transition-all border ${
              isInterested
                ...
                : 'bg-[#F5F1EC] text-[#1A1A1A] border-[#E5E0D8] hover:border-[#1A1A1A]'
            }`}
          >
